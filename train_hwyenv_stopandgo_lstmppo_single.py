...
HighwayStopEnvLSTM : POMDP baseline; only raw kinematics + normalised
                     ...
                     Trained with RecurrentPPO (sb3_contrib, BPTT T=64).
...
class HighwayStopEnvRM(gym.Wrapper):
    ...
    def __init__(self, env, rm, delta_t=None, a_max=6.0, a_min=-6.0, a_min_c=-5.0):
        ...
        # Task Config
        self.STOP_DIST_OFFSET = 150.0 # Virtual line is 150m ahead of start
        self.STOP_SPEED_THRESH = 0.1  # m/s - relaxed
        # Distance from center where the stop line is located
        self.STOP_ZONE_START = 0.0
        self.STOP_ZONE_END = 20.0  # Stop zone
        self.GOAL = 50.0
        self.STOP_REGISTRATION_DISTANCE = 5
        self.REACT_TO_BRAKE = self.STOP_ZONE_END  # [--]
    ...
    def step(self, action):
        ...
        # update the RM state
        # Convert true_props to a frozenset for proper DNF evaluation
        ...
        if self.current_u_id == 0 and (true_props in ['AB']):
            err_AB = d_inq - d_rss
    ...
    def L(self, state, info):
        ...
        if dist_s <= -(self.GOAL - self.STOP_ZONE_START): return 'g'
...
class HighwayStopEnvLSTM(gym.Wrapper):
    ...
    Reward       — mirrors the RM-file scale (stop=+50, goal=+100,
                   ...
                   · Approach  (dist_s > 0): RSS shaping + potential-based
                     progress (identical to RM state-0 formula).
    ...
    Policy       — RecurrentPPO (sb3_contrib) with MlpLstmPolicy:
                     ...
                     Actor head        : Linear(128, n_actions)   [categorical]
                     Critic head       : Linear(128, 1)            [V(h_t)]
                   BPTT truncation horizon T=64 steps ≈ 4.3 s at 15 Hz.
    """
    ...
    def __init__(self, env, delta_t=None, a_max=6.0, a_min=-6.0, a_min_c=-5.0):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        # ── 2. Approach: RSS shaping + potential-based progress ───────────────
        elif dist_s > 0:
            err_AB = d_i - d_rss
            ...
            # Stop-zone event: one-time reward for first full stop (POMDP hidden state)
            if in_zone and is_halted and not self.has_stopped:
                self.has_stopped = True
                event   = 's'
                reward += self.R_STOP

        # ── 3. Crossed stop line without having stopped → severe penalty ──────
        elif dist_s < 0 and self.prev_dist_s >= 0 and not self.has_stopped:
            event   = 'p'
            reward  = self.R_PASSED
            rm_done = True

        # ── 4. Past stop line with prior stop: speed reward + goal check ──────
...
if __name__ == "__main__":
    ...
    # False -> RM                 (HighwayStopEnvRM   + standard PPO)
    USE_LSTM  = True
    store_gif = False
    pre_train = True
    learn_itrs = 2048 * 50   # 100 k timesteps
    ...
    # 5 vehicles × 4 features = 20-dim kinematics.  HighwayStopEnvLSTM appends
    # 1 normalised-distance scalar → 21-dim observation for the LSTM policy.
    ...
    if USE_LSTM:
        ...
        model_dir   = f"lstm_ppo_hwy_sg_{int(learn_itrs)}"
        ...
        if pre_train:
            # Architecture: Dense(256) -> ReLU→Dense(128) -> ReLU→LSTM(128) -> heads
            # (arXiv 2308.05937)
            # n_steps=2048 at 5 Hz gives ~20 episodes per rollout; each episode
            # is treated as one BPTT sequence.  With duration=20 s the natural
            # sequence length is <=100 steps; setting batch_size=64 ensures
            # sub-sequences of exactly T=64 steps for gradient computation,
            # giving ≈12.8 s of temporal context (cf. paper's T=64 at 15 Hz).
            policy_kwargs = dict(
                net_arch        = [256, 128],   # shared dense layers before LSTM
            ...
            model = RecurrentPPO(
                ...
                learning_rate  = 3e-4,
                n_steps        = 2048,
                batch_size     = 64,            # BPTT truncation horizon T=64
                n_epochs       = 10,
                gamma          = 0.99,
                gae_lambda     = 0.95,
                clip_range     = 0.2,
    ...
    denom = max(n_total - n_passed, 1)
    print(f"Avg. d_stop:             {sum_dist_s/denom:.2f} m")
    print(f"{'-'*60}")
